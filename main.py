...
def find_product(update):
    ...
    for row in product_rows:
        ...
        keyboard = [[InlineKeyboardButton("Подписаться", callback_data=json.dumps({'operation': 'subscribe', 'product_name': 'test', 'product_id': row[0]})),
            InlineKeyboardButton("Отписаться", callback_data=json.dumps({'operation': 'unsubscribe', 'product_name': 'test', 'product_id': row[0]}))]]
